...
const CreatePetitionKiswahili = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    reasons: [''],
    efforts: [''],
    that4: 'masuala ambayo ombi hili linahusu hayako mbele ya mahakama yoyote, au chombo chochote cha kikatiba au kisheria.',
    prayers: [''],
  ...
  // Steps configuration
  const steps = [
    { id: 1, title: 'Aina ya Ombi' },
    { id: 2, title: 'Maelezo ya Ombi' },
    { id: 3, title: 'Sababu' },
    { id: 4, title: 'Juhudi Zilizofanywa' },
    { id: 5, title: 'Maelezo ya Mahakama & Maombi' },
    { id: 6, title: 'Tarehe & Taarifa za Mwombaji' },
    { id: 7, title: 'Pitia & Tuma' }
  ];
  ...
  const nextStep = () => {
    ...
    if (currentStep === 2 && !formData.regarding.trim()) {
      setErrors({
        ...errors,
        regarding: 'Tafadhali weka mada ya ombi lako'
      });
      return;
    }
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(14);
    const title = `OMBI LA UMMA KWA BUNGE LA TAIFA KUHUSU ${formData.regarding || '____'}`;
    y = addWrappedText(doc, title, marginLeft, y, maxWidth, 18);

    // Undersigned
    doc.setFont('times', 'normal');
    doc.setFontSize(12);
    const undersigned = `${formData.petitionType === 'individual' ? 'Mimi, NILIYESAINI HAPA CHINI,' : 'SISI, TULIOSAINI HAPA CHINI,'} ${formData.petitioners || ''}`;
    ...
    if (formData.reasons.some(reason => reason.trim() !== '')) {
      y = addWrappedText(doc, 'TUNAVUTA hisia za Bunge kwa yafuatayo:', marginLeft, y, maxWidth);

      // Add reasons
      formData.reasons.forEach((reason) => {
        if (reason.trim()) {
          y = addWrappedText(doc, `KWAMBA, ${reason}`, marginLeft, y, maxWidth);
        }
      });
    }
    ...
    if (formData.efforts.some(effort => effort.trim() !== '')) {
      ...
      formData.efforts.forEach((effort) => {
        if (effort.trim()) {
          y = addWrappedText(doc, `KWAMBA, ${effort}`, marginLeft, y, maxWidth);
        }
      });
    }
    ...
    if (formData.that4 && formData.that4.trim() !== '') {
      y = addWrappedText(doc, `KWAMBA, ${formData.that4}`, marginLeft, y, maxWidth);
    }

    // THEREFORE
    if (formData.prayers.some(prayer => prayer.trim() !== '')) {
      y = addWrappedText(doc, 'KWA HIVYO, Waombaji wenu wanyenyekevu wanaomba Bunge la Taifa kupitia Kamati ya Maombi ya Umma:', marginLeft, y, maxWidth);
    ...
    // Date
    const dateStr = `Imetolewa tarehe ${formData.day} mwezi wa ${formData.month} 20${formData.year}`;
    y = addWrappedText(doc, dateStr, marginLeft, y, maxWidth);

    // Add "And your PETITIONERS will ever pray"
    y = addWrappedText(doc, 'Na WAOMBAJI wenu wataomba daima', marginLeft, y, maxWidth);
    y += 10; // Add some space

    // Add "Presented by" title
    y = addWrappedText(doc, 'Imewasilishwa na:', marginLeft, y, maxWidth);
    y += 5; // Add small space

    // Individual or Group
    if (formData.petitionType === 'individual') {
      [
        `Jina: ${formData.indName || '__________________'}`,
        `Sahihi: ${formData.indSign || '__________________'}`,
        `Mawasiliano: ${formData.indContact || '__________________'}`
      ].forEach(text => {
      ...
      autoTable(doc, {
        head: [['S/No', 'JINA LA MWOMBAJI', 'NAMBARI YA KITAMBULISHO', 'MAWASILIANO', 'SAHIHI']],
        body: rows,
        startY: y,
        theme: 'grid',
  ...
  const handleSubmit = async () => {
    if (!signedPetition) {
      setUploadError('Tafadhali chagua faili la kupakia');
      toast.error('Tafadhali chagua faili la kupakia');
      return;
    }
    ...
    try {
      ...
      // Show success toast
      toast.success('Ombi limetumwa kikamilifu!');
  ...
  const renderStepContent = (step) => {
    switch (step) {
        ...
        return (
          <>
            <Card>
              ...
              <FormGroup>
                <label>Kuhusu (Mada ya Ombi) *</label>
                <StyledInput
                  type="text"
                  name="regarding"
                  value={formData.regarding}
                  onChange={handleChange}
                  placeholder="Weka mada ya ombi lako..."
              ...
              <FormGroup>
                <label>Hapa, tambulisha kwa ujumla mwombaji au waombaji ni kina nani, kwa mfano, raia wa Kenya, wakazi wa kaunti au eneo, wafanyakazi wa sekta fulani, n.k. *</label>
                <StyledTextarea
                  name="petitioners"
                  value={formData.petitioners}
                  onChange={handleChange}
                  placeholder="Weka taarifa za waombaji"
                  required
                />
              </FormGroup>
            </Card>
      ...
      case 3:
        return (
          <Card>
            <h3>Sababu za Ombi</h3>
            <p style={{ color: '#6c757d', marginTop: '-15px', marginBottom: '15px', fontSize: '0.9em' }}>
              Hapa, eleza kwa ufupi sababu za kuomba Bunge liingilie kati kwa kuelezea malalamiko au matatizo kwa kufupisha ukweli ambao mwombaji au waombaji wanataka Bunge lizingatie.
            </p>
            {formData.reasons.map((reason, index) => (
              <FormGroup key={`reason-${index}`}>
                <div style={{ display: 'flex', gap: '10px', marginBottom: '5px' }}>
                  <span>Sababu {index + 1}</span>
                ...
                <StyledTextarea
                  ...
                  placeholder={`Weka sababu ${index + 1}...`}
      ...
      case 4:
        return (
          <Card>
            <h3>Juhudi Zilizofanywa</h3>
            <p style={{ color: '#6c757d', marginTop: '-15px', marginBottom: '15px', fontSize: '0.9em' }}>
              Hapa thibitisha kwamba juhudi zimefanywa ili suala hilo lishughulikiwe na chombo husika, na likashindikana kutoa majibu ya kuridhisha.
            </p>
            {formData.efforts.map((effort, index) => (
              <FormGroup key={`effort-${index}`}>
                <div style={{ display: 'flex', gap: '10px', marginBottom: '5px' }}>
                  <span>Juhudi {index + 1}</span>
                ...
                <StyledTextarea
                  ...
                  placeholder={`Eleza juhudi ${index + 1}...`}
                  style={{ minHeight: '80px' }}
                />
              </FormGroup>
            ))}
      ...
      case 5:
        return (
          <>
            <Card>
              <h3>Maelezo ya Mahakama</h3>
              <p style={{ color: '#6c757d', marginTop: '-15px', marginBottom: '15px', fontSize: '0.9em' }}>
                Hapa thibitisha kwamba masuala ambayo ombi hili linahusu hayako mbele ya mahakama yoyote, au chombo chochote cha kikatiba au kisheria.
              </p>
              <FormGroup>
                <StyledTextarea
                  name="that4"
            ...
            <Card>
              <h3>Maombi</h3>
              <p style={{ color: '#6c757d', marginTop: '-15px', marginBottom: '15px', fontSize: '0.9em' }}>
                Hapa, eleza ombi kwa kueleza kwa muhtasari hatua ambayo waombaji wanataka Bunge lichukue au liepuke kuchukua.
              </p>
      ...
      case 6:
        return (
          <Card>
            <h3>Tarehe & Taarifa za Mwombaji</h3>
            <FormGroup>
              <label>Tarehe ya Ombi</label>
              ...
              <>
                ...
                <FormGroup>
                  ...
                  <StyledInput
                    ...
                    placeholder="Sahihi itaongezwa hapa"
                  />
                </FormGroup>
                <FormGroup>
                  <label>Taarifa za Mawasiliano</label>
                  <StyledInput
                    type="text"
                    name="indContact"
                    value={formData.indContact}
                    onChange={handleChange}
                    placeholder="Simu/Barua pepe"
                    required
                  />
                </FormGroup>
              </>
              ...
              <>
                ...
                {formData.groupPetitioners.map((petitioner, index) => (
                  <div key={index} style={{ marginBottom: '20px', padding: '15px', border: '1px solid #eee', borderRadius: '5px' }}>
                    <FormGroup>
                      <label>Jina la Mwombaji {index + 1}</label>
                      <StyledInput
                        type="text"
                    ...
                    <FormGroup>
                      <label>Nambari ya Kitambulisho/Pasipoti</label>
                      <StyledInput
                        type="text"
                        value={petitioner.id}
                        onChange={(e) => handleGroupPetitionerChange(index, 'id', e.target.value)}
                        placeholder="Nambari ya Kitambulisho/Pasipoti"
                      />
                    </FormGroup>
                    <FormGroup>
                      <label>Taarifa za Mawasiliano</label>
      ...
      case 7:
        return (
          <Card>
            <h3>Pitia Ombi Lako</h3>
            <p>Tafadhali pitia taarifa zote kabla ya kutuma.</p>
            <FormGroup>
              <label>Aina ya Ombi:</label>
              <p>{formData.petitionType === 'individual' ? 'Kibinafsi' : 'Kikundi'}</p>
            </FormGroup>
            <FormGroup>
              <label>Kuhusu:</label>
            ...
            <FormGroup>
              <label>Waombaji:</label>
              <p>{formData.petitioners}</p>
            </FormGroup>
            <FormGroup>
              <label>Tarehe:</label>
              <p>{`Imetolewa tarehe ${formData.day} mwezi wa ${formData.month} 20${formData.year}`}</p>
            </FormGroup>
            <FormGroup>
              <label>Sababu:</label>
              <ul>
            ...
            <FormGroup>
              <label>Juhudi Zilizofanywa:</label>
              <ul>
                {formData.efforts.map((effort, index) => (
            ...
            <FormGroup>
              <label>Maelezo ya Mahakama:</label>
              <p>{formData.that4}</p>
            </FormGroup>
            <FormGroup>
              <label>Maombi:</label>
  ...
  const renderSubmitDialog = () => (
    <>
      ...
      <SubmitDialog>
        <h3 style={{ marginTop: 0, color: '#2c3e50' }}>Pakia Ombi Lililosainiwa</h3>
        <p style={{ marginBottom: '20px', color: '#6c757d' }}>Tafadhali pakia nakala ya ombi lako iliyosainiwa.</p>
        ...
        <div style={{
          ...
          {isUploading && (
            <div style={{
              ...
              <span style={{ marginLeft: '10px' }}>Inapakia...</span>
            </div>
          )}
          <input
            type="file"
          ...
          >
            {signedPetition ?
              `Imechaguliwa: ${signedPetition.name}` :
              'Bofya kuchagua faili au buruta na uweke hapa'}
          </label>
          {uploadError && (
        ...
        <div style={{
          ...
          <strong>⚠️ Muhimu:</strong> Ukishatuma, hutaweza kufanya mabadiliko zaidi kwenye ombi hili.
        </div>
  ...
  return (
    <>
      ...
      <Container>
        <FormSection>
          <FormHeader>
            <h4>Unda Ombi Jipya</h4>
            <div style={{ display: 'flex', gap: '10px' }}>
              <PreviewButton onClick={() => setShowPreview(!showPreview)}>
                {showPreview ? 'Ficha Hakiki' : 'Onyesha Hakiki'}
              </PreviewButton>
            </div>
          </FormHeader>
        ...
        <PreviewSection>
          <PreviewToggle onClick={() => setShowPreview(!showPreview)}>
            <FaEye />
            <span>{showPreview ? 'Ficha Hakiki' : 'Onyesha Hakiki'}</span>
          </PreviewToggle>
          ...
          {showPreview ? (
            ...
            <PreviewPlaceholder>
              ...
              <p>Bofya "Onyesha Hakiki" kuona jinsi ombi lako litakavyoonekana</p>
            </PreviewPlaceholder>
          )}
        </PreviewSection>
...
export default CreatePetitionKiswahili;