...
public class GetDocumentSharedUsersRolesQueryHandler(IDocumentRepository _documentRepository, IUserNotificationRepository _userNotificationRepository) : IRequestHandler<GetDocumentQuery, ServiceResponse<DocumentDto>>
{
    public async Task<ServiceResponse<DocumentDto>> Handle(GetDocumentQuery request, CancellationToken cancellationToken)
    {
        var entity = await _documentRepository.GetDocumentSharedUsersRolesById(request.Id);
